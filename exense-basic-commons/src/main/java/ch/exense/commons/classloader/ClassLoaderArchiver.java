...
public class ClassLoaderArchiver {
    ...
    public static void createArchive(File outputFile, Predicate<String> filter) throws IOException {
        ...
        try (JarOutputStream jos = new JarOutputStream(
            ...
            for (File file : classpathFiles) {
                addFileToArchive(file, jos, addedEntries, filter);
            }
        }
    }
     ...
     */
    private static void addFileToArchive(File file, JarOutputStream jos, Set<String> addedEntries, Predicate<String> filter) throws IOException {
        if (file.isDirectory()) {
            addDirectoryToArchive(file.toPath().toAbsolutePath().normalize(), jos, addedEntries, filter);
        } else if (file.getName().endsWith(".jar")) {
            addJarToArchive(file, jos, addedEntries, filter);
        }
    }
    ...
    private static void addDirectoryToArchive(Path root, JarOutputStream jos, Set<String> addedEntries, Predicate<String> filter) throws IOException {
        Files.walkFileTree(root, new SimpleFileVisitor<Path>() {
            ...
            public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
                ...
                if (filter.test(entryName) && !shouldExcludeFromJar(entryName) && addedEntries.add(entryName)) {
     ...
     *                     or writing to {@code jos}
     */
    private static void addJarToArchive(File jarFile, JarOutputStream jos, Set<String> addedEntries, Predicate<String> filter) throws IOException {
    ...
    public static Predicate<String> getResourceFilter() {
        return ClassLoaderArchiver::isResourceFile;
    }
    ...
    private static boolean isResourceFile(String entryName) {
        // Skip .class files
